...
async function makeBackup() {
  try {
    ...
    // Optionally upload backup to S3 if configured
    try {
      if (process.env.BACKUP_S3_BUCKET) {
        await uploadBackupToS3(out).catch(e=>{ console.error('S3 upload failed', e); });
      }
    } catch(e) { console.error('makeBackup S3 upload err', e); }
  } catch (e) { console.error('makeBackup error', e); }
}

// Schedule regular backups
// Initialize optional S3 client for offsite backups (if configured)
let s3Client = null;
let S3PutObject = null;
if (process.env.BACKUP_S3_BUCKET) {
  try {
    const { S3Client, PutObjectCommand } = require('@aws-sdk/client-s3');
    const s3Region = process.env.AWS_REGION || 'us-east-1';
    const s3Opts = { region: s3Region };
    ...
    console.log('S3 backup configured for bucket:', process.env.BACKUP_S3_BUCKET);
  } catch (e) {
    console.warn('S3 client not available; install @aws-sdk/client-s3 to enable S3 backups', e && e.message);
  }
}

async function uploadBackupToS3(localPath) {
  if (!s3Client || !S3PutObject) return;
  const bucket = process.env.BACKUP_S3_BUCKET;
  const prefix = process.env.BACKUP_S3_PREFIX || '';
  try {
    const data = await fs.readFile(localPath);
    const gz = await gzip(data);
    const keyBase = path.basename(localPath) + '.gz';
    const key = prefix ? (prefix.replace(/\/+$/,'') + '/' + keyBase) : keyBase;
    const params = {
      Bucket: bucket,
      Key: key,
      Body: gz,
    ...
    // Allow optional public flag (not recommended) via env
    if (process.env.BACKUP_S3_PUBLIC === 'true') params.ACL = 'public-read';
    const cmd = new S3PutObject(params);
    await s3Client.send(cmd);
    console.log('Uploaded backup to S3:', bucket + '/' + key);
  } catch (e) {
    console.error('uploadBackupToS3 failed', e);
  }
}
